...
export const listByStatus = query({
    args: {
        status: v.union(v.literal("pending"), v.literal("processing"), v.literal("packed"), v.literal("shipped"), v.literal("delivered"), v.literal("cancelled")),
    },
    handler: async (ctx, args) => {
        const orders = await ctx.db
            .query("orders")
...
export const markDelivered = mutation({
    args: {
        ...
        deliveredItems: v.array(v.object({
    ...
    handler: async (ctx, args) => {
        ...
        for (const deliveredItem of args.deliveredItems) {
            ...
            if (deliveredItem.deliveredQuantity > remainingQuantity) {
                throw new Error(`Cannot deliver ${deliveredItem.deliveredQuantity} units. Only ${remainingQuantity} units remaining for this item.`);
            }
