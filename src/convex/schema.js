...
export const roleValidator = v.union(v.literal(ROLES.ADMIN), v.literal(ROLES.MANAGER), v.literal(ROLES.WAREHOUSE_STAFF), v.literal(ROLES.SALES));
const schema = defineSchema({
    ...
    clients: defineTable({
        name: v.string(),
        type: v.union(v.literal("hotel"), v.literal("restaurant"), v.literal("cafe"), v.literal("office"), v.literal("other")),
    ...
    orders: defineTable({
        ...
        clientId: v.id("clients"),
        status: v.union(v.literal("pending"), v.literal("processing"), v.literal("packed"), v.literal("shipped"), v.literal("partially_delivered"), // Some items delivered, some pending
    ...
    stockMovements: defineTable({
        productId: v.id("products"),
        locationId: v.id("locations"),
        movementType: v.union(v.literal("inbound"), v.literal("outbound"), v.literal("adjustment"), v.literal("transfer")),
        quantity: v.number(),
        unitType: v.optional(v.string()), // e.g., "Sample 250ml", "1L Bottle", "5L Can"
    ...
    // Alerts
    alerts: defineTable({
        type: v.union(v.literal("low_stock"), v.literal("out_of_stock"), v.literal("overstock")),
        productId: v.id("products"),
        locationId: v.id("locations"),
